...
export class AppComponent {
  ...
  async playOrStop(section: 'task' | 'solution'): Promise<void> {
    ...
    if (isPlayingSolution) {
      // By simply canceling, we rely on the utterance's `onend` event to trigger
      // the next sentence. This is cleaner and avoids race conditions.
      // The `onend` handler already has the logic to advance or stop at the end.
      this.synth.cancel();
    } else {
  ...
  private speakSolutionSentenceByIndex(index: number): void {
    ...
    utterance.onerror = (e) => {
        // When canceling, browsers might fire an error with 'canceled' or 'interrupted'.
        // We must ignore both to prevent an incorrect state reset.
